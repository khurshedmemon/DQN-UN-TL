...
from tl_config import get_config
flags = tf.app.flags
import pdb
...
flags.DEFINE_string('mv', 'DRL_UN_rnd_st_tl_CEL_mv_01_30k_WC_Prb10', 'Model version represents model training episodes, edge-weiths, and network visibility')
# Environment
flags.DEFINE_string('env_name', 'p2p-Gnutella08','The name of graph to use')
flags.DEFINE_integer('scale', 30000,'Number of training episodes')
# Etc
flags.DEFINE_boolean('use_gpu', True, 'Whether to use gpu or not')
flags.DEFINE_string('gpu_fraction', '2/3', 'idx / # of gpu fraction e.g. 1/3, 2/3, 3/3')
flags.DEFINE_string('gpu_id', '1', 'GPU Id to use')
...
flags.DEFINE_float('net_part_vis', 0.1, 'initial network visibility at first round of training or testing')
    ...
    with tf.device('/gpu:' + str(FLAGS.gpu_id)):        
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, allow_soft_placement=True, log_device_placement=True)) as sess:
            ...
            try:
                ...
                #added to compute simulation results based on edge-weights = 0.4
                #for e in g.edges():
                    #g.ep.weight[e] = 0.4
            except:
